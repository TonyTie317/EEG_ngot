...
def compute_confusion_matrix(y_true, y_pred, num_classes=4):
    ...
    return: (num_classes, num_classes) matrix, rows=true, cols=pred
    """
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, adj, y, _, _ in train_loader:
            ...
            # Chuẩn hoá adjacency (thêm self-loop + normalize theo hàng)
            A = adj + torch.eye(adj.size(1), device=adj.device).unsqueeze(0)
            A = A / (A.sum(dim=-1, keepdim=True).clamp_min(1.0))  #A~=D−1(A+I)​row-normalized adjacency matrix có self-loop.
        ...
        if val_acc > best_val_acc:
            ...
            # thông báo nhỏ để biết đã cập nhật best
            print(f"💾 Saved BEST model (val_acc={best_val_acc:.3f}) → stgcn_baseline.pt")

    # Lưu model sau cùng (nếu bạn muốn cả bản cuối)
    ...
    # -----------------------------
    # Gom lại toàn bộ dự đoán val lần cuối
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
        for x, adj, y, _, _ in val_loader:
